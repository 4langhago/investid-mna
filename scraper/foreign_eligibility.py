...
# 2026-09-17 재조정: 위 규칙은 Rp 25억 미만 자산 양도를 전부 '불가'로 봤는데, 사용자의
# 실제 예산(한국돈 1~2억 = Rp 11.6억~23.2억)이 통째로 그 아래라 1차 추천이 항상 0건이 됐다.
# 납입자본 Rp 25억은 '인수가에 얹는 돈'이 아니라 '입금 후 인수대금·운영비로 쓰는 돈'이므로,
# 인수가 ₩1억 이상이면 자본금 ₩2.2억 안에서 인수와 운전자금이 성립한다. 그래서 이 구간은
# '조건부'로 열되, 총 현금과 투자계획(Rp 100억) 요건을 안내에 명시한다.
# ₩1억 미만은 여전히 '불가' - 그 규모에 Rp 100억 투자계획을 내는 구조는 성립하지 않는다.
KRW_TO_IDR = 11.6            # 규모 비교용 근사치(USD/IDR 16,000 · USD/KRW 1,380 기준)
ASSET_DEAL_MIN = 1_160_000_000   # ≈ ₩1억
    ...
    if item.get("subtype") == "akuisisi" or item.get("type") == "bisnis":
        ...
        if not price:
            ...
            return (BLOCKED,
                    ...
                    [])
        elif price < ASSET_DEAL_MIN:
            return (BLOCKED,
                    f"자산 양도 {rupiah_ko(price)}(≈₩{price / KRW_TO_IDR / 1e8:.2f}억) - 신규 PT PMA"
                    " 납입자본(Rp 25억)·투자계획(Rp 100억)에 견줘 규모가 너무 작아 적법한 인수"
                    " 구조가 성립하지 않음(현지인 명의 대여는 무효)",
                    ["이미 같은 업종(KBLI)의 PT PMA 를 보유한 경우에만 그 법인 명의로 검토 가능"])
        elif price < PT_PMA_MIN_PAID_UP:
            # ₩1억~2.2억 구간: 납입자본 Rp 25억을 입금한 뒤 그 돈으로 인수·운전자금을 쓰는
            # 구조가 성립한다. 다만 총 현금과 투자계획 요건을 반드시 안내한다.
            steps.append("PT PMA(외국인투자법인) 설립 후 법인 명의로 인수 - 개인 명의 인수 불가")
            steps.append(f"총 현금 약 Rp 25억(≈₩2.2억) 필요 - 납입자본을 입금한 뒤 인수가"
                         f" {rupiah_ko(price)} 와 운전자금을 그 안에서 지출(입금 후 12개월 인출 제한)")
            steps.append("투자계획 Rp 100억(≈₩8.6억, 3년·설비·운전자금 포함) 제출과 이행 보고(LKPM) 필요"
                         " - 이 규모 매장 하나로는 계획이 과도하므로 증설 계획을 함께 설계할 것")
            ...
            return (CONDITIONAL,
                    f"자산 양도 {rupiah_ko(price)} - PT PMA 설립 시 인수 가능하나 납입자본(Rp 25억)이"
                    " 인수가보다 크므로 총 현금 ₩2.2억 이상 필요",
                    steps)
        steps.append("PT PMA(외국인투자법인) 설립 후 법인 명의로 인수 - 개인 명의 인수 불가")
        steps.append("해당 업종 KBLI 의 외국인 지분 상한을 OSS 에서 먼저 확인")
        if not price:
